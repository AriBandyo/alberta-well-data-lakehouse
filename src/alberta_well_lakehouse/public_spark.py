...
def merge_delta(spark: SparkSession, frame: DataFrame, target: Path) -> None:
    ...
    if DeltaTable.isDeltaTable(spark, str(target)):
        DeltaTable.forPath(spark, str(target)).alias("target").merge(frame.alias("source"), key).whenMatchedUpdateAll().whenNotMatchedInsertAll().execute()
    else:
...
def run(root: Path) -> dict[str, int]:
    ...
    try:
        ...
        bronze = None
        for source in files:
            current = spark.read.option("header", True).schema(SCHEMA).csv(str(source)).withColumn("source_file", F.lit(source.name)).withColumn("ingested_at_utc", F.current_timestamp())
        ...
        merge_delta(spark, bronze.withColumn("municipality_key", F.sha2(F.lower(F.regexp_replace(F.trim("municipality"), "[^A-Za-z0-9]+", "_")), 256)), bronze_target)

        silver = spark.read.format("delta").load(str(bronze_target)).dropDuplicates(["municipality", "year", "metric"]).filter(F.col("value") >= 0).withColumn("source_last_updated", F.to_date("source_last_updated")).withColumn("snapshot_retrieved_at", F.to_date("snapshot_retrieved_at"))
